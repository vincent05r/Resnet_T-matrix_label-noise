...
training_target_ds = asm_ds[0]
version = 'iot_final_b'

training_iteration_eval = 10 #number of training loop

Model =  Resnet.ResNet18 #estimation
Model_2 =  Resnet.ResNet18 #training
...
cifar10_config = {
    'path' : "./dataset/cifar10.npz",
    'dataset' : 'cifar10',
    'n_epoch_training' : 100,  #200/100
    'n_epoch_estimate' : 35,  #25/35/50
    'num_classes' : 10,
    'batch_size' : 64,
    'train_val_split_rate' : 0.8,
    'lr' : 0.0015,  #try 0.001/0.002
    'worker_number' : 1,
    'weight_decay_est' : 1e-5,  #works as a regularizer
    'weight_decay_train' : 1e-5,  #works as a regularizer
    'model' : 'resnet18'
}
...
elif training_target_ds == 'cifar10':
    config_dict = cifar10_config
...
for i in range(1, training_iteration_eval + 1): #random seed from 1 to 10
    ...
    #load training data
    train_data = data_load.cifar_dataset(path=config_dict['path'], train=True, transform=transform_train(config_dict['dataset']), target_transform=transform_target,
                                        random_seed=random_seed)
    
    val_data = data_load.cifar_dataset(path=config_dict['path'], train=False, transform=transform_train(config_dict['dataset']), target_transform=transform_target,
                                        random_seed=random_seed)

    test_data = data_load.cifar_test_dataset(path=config_dict['path'], transform=transform_test(config_dict['dataset']), target_transform=transform_target)
