...
parser.add_argument('--dataset', type = str, help = 'cifar(class = 3), FashionMINIST0.5, FashionMINIST0.6', default = 'cifar')
...
parser.add_argument('--version', type = int, help = 'which version is this', default = 1)
...
if args.dataset == 'cifar':
    args.n_epoch = 200 #unconfirm
    args.n_epoch_estimate = 20 #unconfirm
    args.num_classes = 3 #confirm
    train_data = data_load.cifar_dataset(train=True, transform=transform_train(args.dataset), target_transform=transform_target,
                                         random_seed=args.seed)
    val_data = data_load.cifar_dataset(train=False, transform=transform_test(args.dataset), target_transform=transform_target,
                                       random_seed=args.seed)
    test_data = data_load.cifar_test_dataset(transform=transform_test(args.dataset), target_transform=transform_target)
    estimate_state = True
...
#loss
loss_func_ce = nn.CrossEntropyLoss() #we use CE
...
#mkdir
model_save_dir = args.model_dir + '/' + args.dataset + '/' + 'Version_%s'%(args.version) 

if not os.path.exists(model_save_dir):
    os.system('mkdir -p %s'%(model_save_dir))

prob_save_dir = args.prob_dir + '/' + args.dataset + '/' + 'Version_%s'%(args.version)

if not os.path.exists(prob_save_dir):
    os.system('mkdir -p %s'%(prob_save_dir))

matrix_save_dir = args.matrix_dir + '/' + args.dataset + '/' + 'Version_%s'%(args.version)
...
def main():
    ...
    True_T = tools.transition_matrix_generate(noise_rate=add_on_noise_rate, num_classes=args.num_classes)
    ...
    for epoch in range(args.n_epoch):
        ...
        with torch.no_grad():
            model.eval()
            for batch_x, batch_y in test_loader: #problem
                batch_x = batch_x.cuda()
                batch_y = batch_y.cuda()
                out = model(batch_x, revision=False)
                loss = loss_func_ce(out, batch_y)
